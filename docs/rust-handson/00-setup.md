...
```bash
cd ~/Library/Mobile\ Documents/iCloud~md~obsidian/Documents/vault1/tasks/2026-05-03-rust-handson/
mkdir -p code
cd code
```
...
- [ ] `rustc --version` が動く
- [ ] `cargo new` で雛形を作れる
- [ ] `cargo run` `cargo check` `cargo build --release` の違いを言える
- [ ] `cargo fmt` `cargo clippy` を実行できる
- [ ] `Cargo.toml` の `edition` が何か説明できる
